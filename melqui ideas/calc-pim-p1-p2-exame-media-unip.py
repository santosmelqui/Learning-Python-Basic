...
necessita tirar para finalizar dentro da média proposta pela UNIP (Universidade Paulista).'''
...
while True:  # Este loop é utilizado para repetição do programa quando chega ao fim
    while True:  # Este segundo loop é utilizado caso o aluno erre a digitação das notas
        ...
        correto = str(
            input(f'Suas notas foram {np1}, {np2} e {pim}, correto? s/n ').lower())  # mostrando notas para conferência
    ...
    if pim == 0:
        print(
            "\033[0;30;41mVocê reprovou por ter tirado 0 no PIM\033[m")  # zerar no pim gera uma DP e o aluno só é aprovado após conclusão da mesma.
        ...
        print(
            "\033[0;31;42mVocê passou\033[m")  # condição referente ao primeiro if e seus elif's que possibilitam notas abaixo de 7, enquanto este else traz as notas >= 7

    repetir = str(input(
        "Deseja voltar ao início? s/n ").lower())  # variável ref as primeiras linhas do código, para possível repetição do programa
    if repetir != 's':
